...
export const SyncModal: React.FC<SyncModalProps> = ({
  ...
  const syncUrl = baseUrl && syncRoom ? buildSyncQrPayload(syncRoom) || baseUrl : '';
  ...
  const handleSaveRoom = (e: React.FormEvent) => {
    ...
    onSetSyncRoom(clean);
    showNotice(isRu
      ? 'Комната применена. Отсканируйте QR-код на другом устройстве — синхронизация начнется сразу.'
      : 'Room applied. Scan the QR on the other device — sync starts immediately.');
  };
              ...
              <div className="relative p-2 bg-slate-950 rounded-lg border border-cyan-800/60 shadow-inner">
                {qrApiUrl ? (
                  ...
                  <div className="w-40 h-40 flex items-center justify-center text-[11px] text-slate-500 px-3 text-center">
                    {isRu
                      ? 'Введите комнату, чтобы показать QR-код'
                      : 'Enter a room to show the QR code'}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <p className="text-slate-300 leading-relaxed">
                  {isRu
                    ? '1. Откройте эту ссылку или отсканируйте QR-код на телефоне — он настроит комнату автоматически.'
                    : '1. Open this link or scan the QR code on your phone — it sets the room automatically.'}
                </p>
                <p className="text-slate-300 leading-relaxed">
                  {isRu
